...
client.on('message', message => {
const prefix = '#'	
    if(message.content === prefix + 'ccadmin') {
...
client.on('message',async message => {
  ...
  if(args[0] === `${prefix}top`) {
    ...
    topRoyale.setFooter(`GaintPlus Bot | بوت جاينت بلس`, message.guild.iconURL);
...
client.on("ready", () => {
    var guild;
    while (!guild)
        guild = client.guilds.get("500241063001194507")
...
client.on("guildMemberAdd", (member) => {
    let channel = member.guild.channels.find('name', 'cmd-admin');
    if (!channel) {
        ...
        data.forEach((Invite, key, map) => {
            ...
            if (dat[Inv])
                if (dat[Inv] < Invite.uses) {
                    console.log(3);
 channel.send(`Welcome ${member} To Royal Community`)
